...
func main() {
	template := flag.String("template", "update/README_template.md", "Template path for the README.md")
	...
	profileBytes, err := os.ReadFile("update/profile.yml")
	if err != nil {
		log.Fatalf("Failed to read profile.yml: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to read README template: %v", err)
	}
...
func makeProjectMarkdown(projects []project) string {
	...
	for _, project := range projects {
		...
		if strings.HasPrefix(project.Link, "https://github.com/") {
			...
			if err != nil {
				log.Printf("Failed to repo's star count: %v", err)
			} else if starCount != 0 {
...
func getRepoStarCount(link string) (int64, error) {
	...
	if err != nil {
		return 0, errors.WithMessagef(err, "unmarshal")
	}
